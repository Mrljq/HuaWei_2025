...
class HashCollection:
    ...
    def delete_min(self, num,block_num,time,disks):
        ...
        to_remove=[]
        delete_score=0
        list_obj=self.hash_map[num]
        # print(len(list_obj),file=sys.stderr)
        for obj in list_obj:
            #这里会改变obj的某些值
            ...
            #    print('ss',well_list,file=sys.stderr)
...
def a_star(disks,read_queue,timestamp,obj_state):
    ...
    while True:
        ...
        # for disk in disks:
        #     print(disk.do_nothing,file=sys.stderr)
        # print(f'最优的状态分数：{value:07.3f}，状态是：{state}',file=sys.stderr)
        #根据state更新状态
        ...
        # if c==2:
            # print(len(neighbors),file=sys.stderr)
...
def updata_all_state(state,disks,read_queue,A_path,timestamp,obj_state):
    ...
    for disk in disks:
        ...
        if state[disk.id]==0 :
            ...
            # well_list,_=read_queue.delete_min(obj_id,target_block_id,timestamp,disks)
...
def is_goal_state(disks,read_queue):
    ...
    # if is_goal_of_diskdonothing :   
        # print("do nothing",disks[4].do_nothing,file=sys.stderr)
    #如果请求队列为空
...
def generate_valid_combinations(disks, read_queue,timestamp):
    ...
    # print(valid_actions,file=sys.stderr)
...
def process_chunk(chunk,  disks, read_queue):
    ...
    # print(len(chunk),file=sys.stderr)
...
def read_action(timestamp,read_queue,disks,obj_state):
    ...
    for i in range (1,nRead+1):
        ...
        for disk in disks:
            # print(disk.storge_space[0:30],file=sys.stderr)
            # print(disk.storge_space)
    ...
    if not len(read_queue.hash_map):
        ...
        sys.stdout.flush()
        #返回全局变量
        return read_queue,disks,obj_state
